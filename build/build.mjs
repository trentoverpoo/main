...
const PREVIEWABLE = /\.(jpe?g|png|webp|gif|avif)$/i;
function checkPreview(c, at) {
  ...
  else if (!PREVIEWABLE.test(c.doc)) {
...
function checkCitations(list, where, refLabel) {
  ...
  return list.map((c, i) => {
    ...
    const { url_label, preview: _p, ...rest } = c;
    return { ...rest, url, urlLabel, ...(preview ? { preview: true } : {}) };
  });
}
